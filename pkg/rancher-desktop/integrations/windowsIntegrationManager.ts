...
export default class WindowsIntegrationManager implements IntegrationManager {
  ...
  protected async getLinuxToolPath(distro: string, ...tool: string[]): Promise<string> {
    ...
    try {
      ...
      throw new Error(`__temp_ this message is displayed in the GUi__`);
    }
  }
  ...
  protected async syncDistroSocketProxy(distro: string, shouldRun: boolean) {
    ...
    if (shouldRun && this.settings.WSL?.integrations?.[distro] === true) {
      ...
      console.log(`__syncDistroSocketProxy() executable = ${ executable }`);
      console.log(`__syncDistroSocketProxy() getLinuxToolPath(${ distro }) did not fail`);
